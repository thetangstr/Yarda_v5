...
export default function BeforeAfterSlider({
  ...
  return (
    <div className={`relative rounded-2xl overflow-hidden shadow-2xl ${className}`}>
      {/* Loading State */}
      {isLoading && (
        <div className="absolute inset-0 bg-brand-sage flex items-center justify-center z-10">
      ...
      {hasError ? (
        <div className="aspect-[4/3] bg-gradient-to-br from-brand-sage to-brand-cream flex items-center justify-center">
          <div className="text-center p-8">
            ...
            <p className="text-sm text-neutral-600 mt-2">See the transformation</p>
          </div>
        </div>
      ) : (
        <ReactCompareSlider
          ...
          handle={
            <div className="flex items-center justify-center w-10 h-10 bg-white rounded-full shadow-lg border-4 border-brand-green">
      ...
      {!hasError && !isLoading && (
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-sm px-4 py-2 rounded-full shadow-lg animate-fade-in">
